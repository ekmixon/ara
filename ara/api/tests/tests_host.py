...
class HostTestCase(APITestCase):
    ...
    def test_delete_host(self):
        ...
        request = self.client.delete(f"/api/v1/hosts/{host.id}")
    ...
    def test_partial_update_host(self):
        ...
        request = self.client.patch(f"/api/v1/hosts/{host.id}", {"name": "foo"})
    ...
    def test_get_host(self):
        host = factories.HostFactory()
        request = self.client.get(f"/api/v1/hosts/{host.id}")
    ...
    def test_get_hosts_by_playbook(self):
        ...
        request = self.client.get(f"/api/v1/hosts?playbook={playbook.id}")
    ...
    def test_get_hosts_by_name(self):
        ...
        request = self.client.get(f"/api/v1/hosts?name={host.name}")
        self.assertEqual(1, len(request.data["results"]))
        self.assertEqual(host.name, request.data["results"][0]["name"])
    ...
    def test_get_hosts_by_stats(self):
        ...
        for status in statuses:
            request = self.client.get(f"/api/v1/hosts?{status}__gt=1")
        ...
        for status in statuses:
            request = self.client.get(f"/api/v1/hosts?{status}__lt=1")
            self.assertEqual(1, request.data["count"])
            self.assertEqual(1, len(request.data["results"]))
    ...
    def test_get_host_by_date(self):
        ...
        for field in negative_date_fields:
            request = self.client.get(f"/api/v1/hosts?{field}={past.isoformat()}")
    ...
    def test_get_host_order(self):
        ...
        for field in order_fields:
            request = self.client.get(f"/api/v1/hosts?order={field}")
